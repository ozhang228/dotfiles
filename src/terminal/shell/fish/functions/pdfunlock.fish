...
function pdfunlock
    ...
    function _pdfunlock_one --argument-names src password
        set -l out (string replace -r '\.locked\.pdf$' '.pdf' $src)
        qpdf --decrypt --password="$password" -- "$src" "$out" && rm -f -- "$src"
    end

    if test -d "$argv[1]"
        for f in "$argv[1]"/*.locked.pdf
            echo "Unlocking: $f"
            _pdfunlock_one "$f" "$pw" || return 1
        end
    else
