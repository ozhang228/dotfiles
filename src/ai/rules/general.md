...
- **Vacuous success-shape test:** asserting only that a call succeeded (`isinstance(result, Ok)`, `assert response.status == 200`, `np.isfinite(x)`) when the function has a real, computable expected value. Any wrong-but-successfully-shaped output satisfies these. Replace with the actual pinned value once you know what correct looks like — reserve success/error-shape assertions for cases where no single correct value exists (e.g., "this must error, but any error message is acceptable").
