...
- Default every cell-local variable to a leading underscore (e.g. `_face_value`, `_haircut`); underscore-prefixed names are private to the cell and exempt from the uniqueness rule. Only leave a variable unprefixed if another cell depends on it via `return foo,`.
- All imports go in the `with app.setup:` block. No per-cell imports.
- Give each UI selector its own standalone cell, separate from the logic that consumes it.
...
- When Oscar asks to inspect a notebook, open it with `uv run marimo edit`, not `marimo run` or a static export, so the code remains visible alongside the results. Run it from `~/anvil` with `--host 0.0.0.0 --headless --watch`, keep the server running for review, and report the reachable URL and authentication token. When executing a notebook for results, use `marimo run --include-code` so the served notebook retains its code view too.
- When a cell passes embedded Python source to a subprocess, build the source from quoted lines joined with `"\n"`. Marimo's export can strip indentation inside a multiline string, breaking nested Python blocks; verify execution with `marimo export ipynb --include-outputs`.
