...
12. **Track open questions.** Whenever a decision's rationale isn't evident from the diff, tests, or surrounding code — an unusual data structure, a tradeoff between two reasonable approaches, an abstraction whose necessity isn't obvious, a naming/scope call that looks arbitrary — note it as an open question instead of guessing intent. Skip this for mechanical, self-evidently-motivated changes; most small or routine diffs will surface none.

### Phase 2 — Architecture gate (hard block)

**Author interview, on by default.** If Phase 1 surfaced open questions, ask them first — before stating any verdict. One at a time, prose, same shape as `brainstorm`'s grilling: ask without proposing an answer, so the author reasons through it rather than reacting to a canned recommendation, and say what the answer will change about the verdict. Two things ride on the answer: if it holds up, treat the tradeoff as intentional and documented, not a finding; if it doesn't — the author can't actually explain it, or the reasoning has a real hole — that becomes a finding itself (`Model:` under Bugs, or a Nit, depending on severity), because a decision the author can't defend is worth flagging even when the code runs. Skip this step entirely when Phase 1 found nothing genuinely non-obvious.
