...
- Skip the brainstorm workflow entirely when the task is purely "make ty/mypy happy" or "make the build pass" with **no behavior change and no design choice** — there's a wrong type and a right type, no tradeoffs to weigh. 
...
Use a self-contained local visual plan as the user-facing design artifact when the work has real design space. Read `references/visual-plan.md` in full — it owns the self-contained contract, the shared plain-HTML/CSS component library, and the validation/serving steps. One rule that lives only here: the visual plan is for design approval; `tmp/PLAN.md` (written later, see Workflow below) is for implementation sequencing — keep the two separate.
...
- Write local visual plan - read `references/visual-plan.md` in full. Create `<plan-dir>/plan.html` (plus `plan.md` fallback) per that reference. This is the user-facing review gate. Include the approved approach, tradeoffs, expected behaviors, performance boundaries, file map, and open questions.
...
- User reviews visual plan - report the served URL, local `plan.html` path, and plan folder path, then ask the user to review the visual plan before proceeding.
- Transition to implementation with a Markdown plan - follow the format in `references/writing-plans.md` to produce a step-by-step plan for the agent.
  ...
  - **Bring down the local plan server the moment execution starts.** Once the user approves and asks to implement, the plan is no longer the live review surface — kill the `python3 -m http.server` process serving the plan directory so it isn't left running for the rest of the session. The `plan.html`/`plan.md` files stay on disk for reference; only the server comes down.
