...
- Write local visual plan - read `references/visual-plan.md`, plus `references/document-quality.md` and the visual references needed for the artifact. Create `<plan-dir>/plan.mdx` per that reference. This is the user-facing review gate. Include the approved approach, tradeoffs, expected behaviors, performance boundaries, file map, and open questions.
