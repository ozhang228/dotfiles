...
- Present suggested approach - talk about tradeoffs and if there are any other possible approaches that have different tradeoffs. If correctness depends on a tradeoff phrased as present-tense fact ("X is true in practice"), verify it or flag it as `todo` in the assumption-list — never accept it silently. 
...
- Write local visual plan - read `references/visual-plan.md` in full. Create `tmp/plan.html` per that reference, including its causal explanation and final understanding quiz. Include the approved approach, tradeoffs, expected behaviors, performance boundaries, file map, and open questions.
