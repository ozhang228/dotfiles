---
name: brainstorm
description: You MUST use this before any creative work — creating features, building components, adding functionality, or building a script/tool that has real design space (e.g. "create a script that compares X against Y", "propose how we could do Z") — when there are genuine design choices to explore. Its core is a relentless grilling phase: interview the user one question at a time, with a recommended answer each, until the plan is stress-tested and there's shared understanding. Also fires on grill trigger phrases ("grill me", "grill this plan", "stress-test this", "interview me about this"). Skip when plan mode is already driving a design conversation, when the user is directly steering a concrete change, when the task is a mechanical fix (type errors, build fixes, config edits with no design choices), when the user just wants to try a throwaway/experimental thing in tmp/ (not production work), or when they're asking for a context handoff/summary for another agent rather than a design.
...
- Grill the design - this is the default phase before any coding, and the heart of the skill. Interview the user **relentlessly** about every aspect of the plan until you reach a shared understanding. Walk down each branch of the design tree, resolving dependencies between decisions one-by-one (answer the decision an upstream choice depends on before the choices that hang off it). Don't stop at the first layer of questions; keep going until there's nothing load-bearing left unresolved.
  ...
  - **Default to prose questions.** Reserve `AskUserQuestion` (multi-option picker with previews) for moments where the user is at a decision point between 2-4 mutually exclusive concrete paths, not still exploring. Reaching for the picker too early commits to a frame ("here are the options") before the user has decided what shape the answer should take.
