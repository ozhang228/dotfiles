...
- Write local visual plan - read `references/visual-plan.md`, plus `references/document-quality.md` and the visual references needed for the artifact. Create `<plan-dir>/index.html` and `<plan-dir>/plan.md` using the Local Visual Plan Artifact rules above. This is the user-facing review gate. Include the approved approach, tradeoffs, expected behaviors, file map, and open questions.
- Spec self-review - inspect the generated local files for placeholders, contradictions, ambiguity, scope drift, broken anchors, and missing sections. If a repo-native checker exists for HTML or Markdown, run it; do not install one.
- User reviews visual plan - report the local `index.html` path and plan folder path, then ask the user to review the visual plan before proceeding.
