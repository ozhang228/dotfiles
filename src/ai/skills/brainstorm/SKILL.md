...
- Read code context from local files and shell commands only. Do not fetch schemas, install target-repo packages, call external visual-plan tools, publish to a database, or depend on any external server. The only package setup allowed during this workflow is the global renderer setup owned by `scripts/serve-mdx-visual-plan`.
- Prefer `./tmp/visual-plan-<slug>/` for scratch review artifacts. Use `plans/<slug>/` only when the user explicitly wants the artifact checked in. The folder contains `plan.mdx` as the single authored source. Optional assets must be local files in the same folder.
- Render `plan.mdx` with `scripts/serve-mdx-visual-plan <plan-dir> <port>`. That script owns global renderer setup through the skill's checked-in `assets/mdx-visual-plan-renderer/` package. Any generated HTML, cache folder, or build output is renderer-owned and must not become a second hand-maintained source file.
- Keep the plan concise and scannable: outcome, expected behaviors, performance boundaries, tradeoffs, file map or diagram, and open questions. Use the renderer's MDX components for the first viewport and dense review sections; raw Markdown headings and paragraphs alone are not enough for a visual plan. Add a visual canvas section only when a visual surface helps the user decide; keep backend and architecture plans document-focused with inline diagrams or file maps.
- Validate before handoff without target-repo dependencies: confirm `plan.mdx` exists, open/read it enough to catch broken anchors or placeholders, run `scripts/serve-mdx-visual-plan <plan-dir> <port>`, and run any target-repo formatter/checker only if it already exists locally. Do not install target-repo packages, execute remote packages, or fetch remote schemas.
- Verify the served `http://127.0.0.1:<port>/` URL responds, and report the verified URL with the local folder path and direct `plan.mdx` path.
...
- Write local visual plan - read `references/visual-plan.md`, plus `references/document-quality.md` and the visual references needed for the artifact. Create `<plan-dir>/plan.mdx` using the Local Visual Plan Artifact rules above. This is the user-facing review gate. Include the approved approach, tradeoffs, expected behaviors, performance boundaries, file map, and open questions.
...
- Transition to implementation with a Markdown plan - follow the format in `references/writing-plans.md` to produce a step-by-step plan for the agent.
  ...
  - Save the implementation plan to `cwd/docs/PLAN.md` unless the fast-path rule above makes a separate file redundant. Once the plan is saved, **stop and hand off.** The user drives implementation. Wait for them to direct specific steps ("go ahead with task 1", "implement this"). Only execute if they explicitly say so.
