...
<The non-obvious _why_. Link to trading_definitions.md when a term has one.>
...
3. **Prune while you're there.** If the existing entry has any of the "does NOT belong" content, delete it in the same commit. The wiki rule from `GLOBAL.md`: _"If a change contradicts an existing entry, update or remove the old entry."_ The same applies to entries that are merely stale, not contradictory.
